...
<template>
    <component :is="href ? 'a' : 'div'" :href="href" :class="['block', href ? 'hover:no-underline' : '']">
        <Card :class="['transition-all hover:scale-[1.02]', href ? 'cursor-pointer hover:shadow-md' : '']">
            <CardHeader :class="['flex flex-row items-center justify-between space-y-0 pb-2']">
                <CardTitle class="truncate text-sm font-medium">{{ title }}</CardTitle>
                <div :class="['flex h-8 w-8 items-center justify-center rounded-full', colorMap[color].bgLight, colorMap[color].bgDark]">
                    <component :is="icon" :class="['h-4 w-4', colorMap[color].iconLight, colorMap[color].iconDark]" />
                </div>
            </CardHeader>
            <CardContent class="pt-4">
                <div class="flex items-baseline justify-between">
                    ...
                    <div v-if="trend !== undefined" class="flex items-center text-xs">
                        <span class="font-semibold" :class="trendClass"> {{ trend > 0 ? '+' : '' }}{{ trend }}% </span>
                    </div>
                </div>
