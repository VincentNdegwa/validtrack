...
const chartData = computed(() => {
    ...
    const labels = Object.keys(statusCounts).map((status) => statusLabels[status as unknown as keyof typeof statusLabels] || `Status ${status}`);
    const data = Object.values(statusCounts);
    const backgroundColor = Object.keys(statusCounts).map((status) => statusColors[status as unknown as keyof typeof statusColors] || '#CCCCCC');
    ...
    <Card class="h-full shadow-md transition-shadow duration-300 hover:shadow-lg">
        <CardHeader class="border-b pb-2">
            <CardTitle class="flex items-center space-x-2">
                <span class="mr-2 inline-block h-6 w-2 rounded-sm bg-gradient-to-b from-green-500 via-amber-500 to-red-500"></span>
                <span>Document Status Distribution</span>
            </CardTitle>
        </CardHeader>
        <CardContent class="pt-4">
            <div class="h-80">
                <Pie :data="chartData" :options="chartOptions as any" aria-label="Document status distribution chart" />
            </div>
        </CardContent>
    </Card>
</template>
