...
const chartData = computed(() => {
    ...
    const sortedTypes = Object.values(docTypeMap).sort((a: DocTypeCount, b: DocTypeCount) => b.count - a.count);
    ...
    <Card class="h-full shadow-md transition-shadow duration-300 hover:shadow-lg">
        <CardHeader class="border-b pb-2">
            <CardTitle class="flex items-center space-x-2">
                <span class="mr-2 inline-block h-6 w-2 rounded-sm bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500"></span>
                <span>Document Type Distribution</span>
            </CardTitle>
        </CardHeader>
        <CardContent class="pt-4">
            <div class="relative h-80">
                <Doughnut :data="chartData" :options="chartOptions as any" aria-label="Document type distribution chart" />
            </div>
        </CardContent>
    </Card>
</template>
