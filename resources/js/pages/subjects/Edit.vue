...
const form = useForm({
    ...
    subject_type_id: props.subject.subject_type_id?.toString() || '',
    email: props.subject.email || '',
    phone: props.subject.phone || '',
    address: props.subject.address || '',
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: props.subject?.name || 'Edit Subject',
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold text-foreground">Edit Subject: {{ subject.name }}</h1>
            </div>
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <div class="space-y-2">
                            <div class="flex items-center justify-between">
                                ...
                                <Link href="/subject-types" class="text-xs text-primary hover:underline"> Manage Types
                                </Link>
                            </div>
                            <div class="flex space-x-2">
                                <select id="subject_type_id" v-model="form.subject_type_id"
                                    class="w-full rounded-md border border-input bg-background px-3 py-2 text-sm placeholder:text-muted-foreground focus:ring-2 focus:ring-primary focus:outline-none"
                                    :class="{ 'border-red-500': form.errors.subject_type_id }">
                                    <option value="">Select Type</option>
                                    <option v-for="type in props.subjectTypes" :key="type.id" :value="type.id">{{
                                        type.name }}</option>
                                </select>
                            ...
                            <p v-if="form.errors.subject_type_id" class="mt-1 text-xs text-red-500">{{
                                form.errors.subject_type_id }}</p>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <p v-if="form.errors.address" class="mt-1 text-xs text-red-500">{{ form.errors.address }}
                            </p>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <p v-if="form.errors.category" class="mt-1 text-xs text-red-500">{{ form.errors.category }}
                            </p>
                        </div>
